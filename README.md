...
More commands: `scripts/rm.sh test | build | run | marker | pull-maps | devices | clean`. To use Xcode instead, run `scripts/rm.sh gen` and open `RoomMapper.xcodeproj`. Signing is automatic; set your team in `project.yml` if it differs.
...
Packages/MapCore/        pure Swift package: geometry, keyframe policy, voxel maps, codecs, storage, backend, party wire formats; 463 unit tests
App/                     iOS app: Capture (ARKit, keyframe processing), Rendering (Metal), Cloud (backend client, parties, Ably), UI (SwiftUI), Support
...
Marker-based origins are implemented (detection, `world_from_origin`, the `aligned` flag and the status chip). Uploads and collaborative sessions are still in progress; their extension points are documented in docs/ARCHITECTURE.md.
