...
- **Global cloud:** each keyframe is unprojected on the CPU with the depth-scaled intrinsics, colored from the camera image, and inserted into a 2 cm voxel grid (first sample per cell). The cloud is capped at 3 M points; at the cap it is coarsened once to 3 cm and then frozen.
